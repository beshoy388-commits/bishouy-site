...
export default function ArticleDetail() {
  ...
  const renderArticleContent = (content: string) => {
    ...
    // Authenticity Filter: Identify and exclude autogenerated imagery (LoremFlickr/Pollinations)
    // We filter the parts to remove any that contain a directive for an autogenerated image
    const filteredParts: string[] = [];
    for (let i = 0; i < parts.length; i++) {
        const part = parts[i];
        const directiveMatch = part.match(/<!-- img:([a-z]+):(\d+)% -->/);
        
        if (directiveMatch && i + 1 < parts.length) {
            const nextPart = parts[i + 1];
            const isAutoGenerated = nextPart.includes('loremflickr.com') || nextPart.includes('pollinations.ai');
            if (isAutoGenerated) {
                // Skip both the directive AND the image markdown part
                i++; 
                continue;
            }
        }
