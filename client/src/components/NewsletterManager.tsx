...
export default function NewsletterManager() {
  ...
  const activeSubscribersCount =
    listQuery.data?.filter((s: any) => s.active).length || 0;
              ...
              <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (url)
                          insertText(
                            `<a href="${url}" style="color: #E8A020; text-decoration: underline;">`,
                            "</a>"
                          );
                      }}
                      className="p-1.5 hover:bg-[#2A2A28] text-[#8A8880] hover:text-[#F2F0EB] rounded-sm transition-colors"
                ...
                {showUploader && (
                  <div className="mb-4 p-4 bg-[#0F0F0E] border border-[#2A2A28] rounded-sm">
                    <p className="text-[10px] font-ui text-[#8A8880] uppercase tracking-widest mb-3">
                      Upload image to get URL
                    </p>
                    <ImageUploader
                      onImageUpload={(url: string) => {
                        insertText(
                          `<img src="${url}" alt="Newsletter Image" style="max-width: 100%; height: auto; border-radius: 4px; display: block; margin: 20px 0;" />\n`
                        );
                ...
                <p className="text-xs text-[#8A8880] mt-1">
                  Email will be automatically wrapped in the Bishouy.com
                  template.
                </p>
              </div>
          ...
          <Card className="bg-[#1C1C1A] border-[#2A2A28] p-6">
            ...
            {historyQuery.isLoading ? (
              <div className="py-8 flex justify-center">
                <Loader2 className="animate-spin text-[#E8A020]" />
              </div>
            ) : (
        ...
        <div className="space-y-6">
            ...
            {listQuery.isLoading ? (
              ...
              <div className="space-y-2">
                {listQuery.data?.map((sub: any) => (
                  <div
                    key={sub.id}
                    className="group flex items-center gap-3 py-3 border-b border-[#2A2A28] last:border-0"
                  >
                    <Mail
                      size={14}
                      className={
                        sub.active ? "text-green-500" : "text-gray-600"
                      }
                    />
                    <div className="min-w-0 flex-1">
                      <p className="text-sm text-[#F2F0EB] truncate">
                        {sub.email}
                      </p>
                ...
                {listQuery.data?.length === 0 && (
                  <p className="text-xs text-[#555550] text-center py-4">
          ...
          <div className="bg-[#E8A020]/10 border border-[#E8A020]/20 rounded-sm p-4">
            <p className="text-xs text-[#8A8880] leading-relaxed">
              <strong className="text-[#E8A020]">Tip:</strong> Use the editor
              toolbar to insert images and formatting. Always test the
              newsletter on yourself before sending to the full list.
            </p>
          </div>
        </div>
      </div>
