/*
 * BISHOUY.COM — Rich Text Editor
 * Markdown-based editor with toolbar, image insertion with position/size controls, and live preview
 */
...
import {
  ...
  Quote, Code, Link as LinkIcon, Image, Eye, Edit2, Minus,
...
export default function RichTextEditor({ value, onChange, placeholder, maxLength = 50000 }: RichTextEditorProps) {
  ...
  const toolbarActions = [
    ...
    { icon: Image, label: "Image", action: () => setImageDialog({ ...imageDialog, open: true }) },
  ];
    ...
    <div className="space-y-0">
      ...
      {showPreview ? (
        ...
        <textarea
          ...
          maxLength={maxLength}
          rows={20}
          placeholder={placeholder || "Write your article using Markdown...\n\n# Heading\n**Bold** *Italic*\n\nUse the toolbar to format text and insert images."}
          className="w-full bg-[#0F0F0E] border border-t-0 border-[#2A2A28] text-[#F2F0EB] font-mono text-sm px-4 py-3 rounded-b-sm focus:outline-none focus:border-[#E8A020] transition-colors min-h-[400px] resize-y"
        />
      )}

      {/* Character Count */}
      <div className="text-right">
        <span className="font-ui text-xs text-[#555550]">{value.length}/{maxLength}</span>
      </div>

      {/* Image Insert Dialog */}
      {imageDialog.open && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" onClick={() => setImageDialog({ ...imageDialog, open: false })}>
          <div className="bg-[#1C1C1A] border border-[#2A2A28] rounded-sm p-6 w-full max-w-lg mx-4" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-display text-xl text-[#F2F0EB] mb-6">Insert Image</h3>

            <div className="space-y-4">
              {/* Image URL */}
              <div>
                <label className="block text-[#8A8880] text-sm mb-1">Image URL</label>
                <input
                  type="url"
              ...
              {/* Alt Text */}
              <div>
                <label className="block text-[#8A8880] text-sm mb-1">Alt Text</label>
                <input
                  type="text"
              ...
              {/* Caption */}
              <div>
                <label className="block text-[#8A8880] text-sm mb-1">Caption (optional)</label>
                <input
                  type="text"
              ...
              {/* Position */}
              <div>
                <label className="block text-[#8A8880] text-sm mb-2">Position</label>
                  ...
                  {[
                    ...
                    return (
                      <button
                        key={pos.value}
                        type="button"
                        onClick={() => setImageDialog({ ...imageDialog, position: pos.value as any })}
              ...
              {/* Width */}
              <div>
                <label className="block text-[#8A8880] text-sm mb-1">Width: {imageDialog.width}%</label>
                <input
                  type="range"
                  min="20"
                  max="100"
                  ...
                  onChange={(e) => setImageDialog({ ...imageDialog, width: e.target.value })}
              ...
              {imageDialog.url && (
                <div className="border border-[#222220] rounded-sm p-3">
                  ...
                  <div className={`${
                    imageDialog.position === "left" ? "text-left" :
                    imageDialog.position === "right" ? "text-right" :
                    "text-center"
                  }`}>
                    <img
                      src={imageDialog.url}
                      alt={imageDialog.alt}
                      style={{ width: `${imageDialog.width}%`, maxHeight: "200px", objectFit: "cover" }}
                    ...
                    />
                    {imageDialog.caption && (
                      <p className="text-[#8A8880] text-xs mt-1 italic">{imageDialog.caption}</p>
                    )}
                  </div>
                </div>
              )}
