...
export default function GlobalComments() {
  ...
  const isLoading =
    filter === "pending" ? pendingQuery.isLoading : allQuery.isLoading;
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
        <div>
          ...
          <p className="font-ui text-xs text-[#8A8880]">
            Global moderation and community management
          </p>
        </div>
        ...
        <div className="space-y-4">
          {commentsData.map((comment: any) => {
            ...
            return (
              <Card
                key={comment.id}
                className="bg-[#1C1C1A] border-[#2A2A28] p-5 hover:border-[#333330] transition-colors relative"
              >
                ...
                <div className="flex flex-col md:flex-row items-start justify-between gap-4 pl-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center flex-wrap gap-x-3 gap-y-2 mb-3">
                      <span
                        className={`flex items-center gap-1 font-ui text-[10px] uppercase tracking-widest px-2 py-1 rounded-sm ${
                          isPending
                            ? "bg-yellow-900/40 text-yellow-500 border border-yellow-500/20"
                            : isApproved
                              ? "bg-green-900/40 text-green-500 border border-green-500/20"
                      ...
                      <div className="flex items-center gap-2">
                        {comment.userAvatarUrl ? (
                          <img
                            ...
                            className="w-5 h-5 rounded-full object-cover border border-[#2A2A28]"
                            alt={displayName}
                          />
                        ) : (
                        ...
                        <span className="font-ui text-xs text-[#E8A020] font-medium">
                          {displayName}
                        </span>
                      </div>

                      <span className="font-ui text-xs text-[#555550] flex items-center gap-1">
                        on Article <ArrowRight size={10} className="inline" /> #
                        {comment.articleId}
                      </span>
                    ...
                    {/* Content */}
                    <div>
                      <p className="text-[#D4D0C8] leading-relaxed text-sm break-words">
                        {comment.content}
                      </p>
                      {comment.isEdited === 1 && comment.originalContent && (
                        <div className="mt-3 p-3 bg-[#0F0F0E] rounded-sm border border-[#222220] flex flex-col gap-1">
                          ...
                          <p className="text-[#8A8880] text-xs italic line-through opacity-70">
