                    ...
                    <form onSubmit={handleSave} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="font-ui text-[10px] font-600 text-[#E8A020] uppercase tracking-widest block">Internal Title</label>
                                <input
                                    name="title"
                            ...
                            <div className="space-y-2">
                                <label className="font-ui text-[10px] font-600 text-[#E8A020] uppercase tracking-widest block">Position</label>
                                <select
                                    name="position"
                    ...
                    {adsQuery.isLoading ? (
                        ...
                        <div className="grid grid-cols-1 divide-y divide-[#2A2A28] bg-[#1C1C1A] border border-[#2A2A28] rounded-sm">
                            {adsQuery.data?.map(ad => (
                                <div key={ad.id} className="p-4 flex flex-col md:flex-row gap-4 items-center justify-between hover:bg-[#0F0F0E]/50 transition-colors">
                                    <div className="flex items-center gap-4 flex-1 w-full">
                                        <img src={ad.imageUrl} alt={ad.title} className="w-24 h-16 object-cover bg-[#0F0F0E] rounded-xs border border-[#2A2A28] flex-shrink-0" />
                                        <div className="min-w-0">
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className={`w-2 h-2 rounded-full ${ad.active ? 'bg-green-500' : 'bg-red-500'}`} title={ad.active ? 'Active' : 'Inactive'} />
                                                <h4 className="font-ui font-600 text-[#F2F0EB] truncate">{ad.title}</h4>
                                            </div>
                                            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-[#8A8880] font-ui">
                                                <span className="uppercase tracking-widest flex items-center gap-1">P: {ad.position.replace('_', ' ')}</span>
                                                <a href={ad.linkUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-[#E8A020] hover:underline truncate max-w-[200px]"><LinkIcon size={10} /> {ad.linkUrl}</a>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-6 flex-shrink-0 w-full md:w-auto mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-[#2A2A28]">
                                        ...
                                        <div className="flex gap-4 mr-4">
                                            <div className="flex flex-col items-center">
                                                <span className="font-ui text-[10px] text-[#555550] uppercase tracking-widest flex items-center"><Eye size={10} className="mr-1" /> Impr</span>
                                                <span className="text-[#F2F0EB] font-headline">{ad.impressionCount || 0}</span>
                                            </div>
                                            <div className="flex flex-col items-center">
                                                <span className="font-ui text-[10px] text-[#555550] uppercase tracking-widest flex items-center"><MousePointerClick size={10} className="mr-1" /> Clicks</span>
                                                <span className="text-[#E8A020] font-headline">{ad.clickCount || 0}</span>
                                            </div>
                                        </div>
                                        {/* Actions */}
